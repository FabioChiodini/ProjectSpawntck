...
HoneypotPortK-i=8081

#Launches additional local
    #docker run -d --name honeypot-$i -p $HoneypotPortK:$HoneypotPortK $HoneypotImageK
    docker run -d --name honeypot-j-e LOG_HOST=$publiciplogstash -e LOG_PORT=$ReceiverPortK -p $HoneypotPortK:$HoneypotPortK-i $HoneypotImageK 
#launches nginx (optional)

echo ----
echo "$(tput setaf 6) Local honeypot RUNNING ON $ipAWSK:$HoneypotPortK-i $(tput sgr 0)"
echo "$(tput setaf 6) Local honeypot sending logs to $publiciplogstash PORT $ReceiverPortK$(tput sgr 0)"
echo "$(tput setaf 4) Open a browser to : $ipAWSK:$HoneypotPortK-i (tput sgr 0)"
echo ----
